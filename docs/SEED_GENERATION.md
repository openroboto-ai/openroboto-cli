...
2. **the competition id** — the `id` of the season the submission was admitted to.
   ...
   payload, and *not* the season ordinal the API shows you;
...
### ⚠️ The second input is the competition id — not `r`, not the season ordinal

🔴 **Taking the raw `r` out of your own commitment payload produces a seed that
does not match.**
...
- **It is not the ordinal the API displays.** That one is the
...
block_hash = "0x" + "11" * 32
competition_id = 1        # positional: the second argument
