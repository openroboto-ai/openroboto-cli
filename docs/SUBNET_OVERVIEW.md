...
For a trace of one real submission with live transaction hashes, see the companion document *E2E Testnet Walkthrough*.
...
upload to Hugging Face ──► burn (eval fee)
                           commitment {repo, commit, burn tx}
                                        │
                                        ▼
                           ChainScanner (60 s poll) ──────────► payment verified? ──no──► rejected
                                                                    │ yes
                                                                    ▼
                                                                seed = f(block hash, drand)
                                                                task enqueued (pending)
                                                                worker evaluates (6 suites)
                                                                scores posted (done / failed)
                                                                ranking resolved
...
| Status | Meaning |
|---|---|
| `unknown` | Announcement seen on chain, not yet verified |
| `pending` | Payment verified, task enqueued for evaluation |
| `done` | Evaluation completed, scores recorded |
| `failed` | Evaluation ran but failed (e.g. corrupted weights); no score, no ranking entry |
| `rejected` | Payment or format verification failed; never enqueued |

One commitment per hotkey per round. Re-submitting overwrites the previous entry and resets it to the start of the pipeline — a miner cannot accumulate multiple queue slots.

### What the uploaded repo must contain

The evaluation worker loads **complete model checkpoints only**. Before any GPU time is spent, every submission goes through a structural pre-check (`libero_eval/check_model.py` in the evaluation repo); a submission that fails it is marked `failed`, and the rejection reason is recorded so the miner can see exactly why.
...
If any check fails, the submission is marked `rejected` and never enters the queue — and the burned TAO is **not refunded**. Payment records are auditable via `/api/v1/payments/*` (admin) and summarized publicly.
...
2. Each subsequent miner **challenges the current champion**: the challenger's average score must exceed the champion's by at least **`champion_margin` (default 0.02)**.
...
| Overfit to a known evaluation seed | Seed derived from block hash + drand, unknowable pre-submission (§5) |
...
| Resubmit someone else's weights under a new name | Weight fingerprinting via HF LFS metadata (sha256 of every shard) — detection-only for now, see note below |
...
| Reuse one burn tx across submissions | Anti-replay check, DB + in-memory (§4) |
...
Two caveats worth stating plainly. Weight fingerprinting currently runs in detection mode: duplicate hashes are logged by the scanner but do not yet auto-reject a submission. And "authenticated" guards the score endpoint against outsiders — the scores themselves are produced and submitted by the operator's own benchmark workers, and independent third-party re-scoring is not part of the protocol yet. What keeps the operator honest today is that the harness is public and every input (weights, seed, suite list) is pinned, so any published score can be reproduced offline.
...
| `GET /api/v1/queue/status` | Evaluation queue: per-task status (pending / evaluating / done / failed) |
...
| Validator guide | [VALIDATOR.md](./VALIDATOR.md) · [VALIDATOR_DEPLOY.md](./VALIDATOR_DEPLOY.md) |
