...
| `huggingface` | `repo_id` | Optional, used verbatim when set. Empty = `<username>/<base_model_family>-<last 12 of hotkey>`, i.e. **one repository per season** — and a workspace whose `competition:` section names no base model keeps its `<username>/pi05-…` name, so upgrading never moves your repository. Set this only to name one the default would not derive |
